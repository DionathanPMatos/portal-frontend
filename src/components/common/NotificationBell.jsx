...
const NotificationBell = () => {
  ...
  const handleNotificacaoClick = async (notificacao) => {
    try {
      await apiClient.patch(`/api/notificacoes/${notificacao.id}/lida`, {
        tipo: notificacao.tipo,
      });
      setNotificacoes((prev) =>
        prev.filter(
          (n) => !(n.id === notificacao.id && n.tipo === notificacao.tipo),
        ),
      );
    } catch (err) {
    ...
    const routes = {
      nova_noticia: "/",
      status: "/crm/visitas",
      feedback: "/crm/visitas",
      retorno: "/crm/visitas",
      aprovacao_gestor: "/crm/visitas",
      nova_pergunta_faq: "/dtc/perguntas",
      resposta_faq: "/dtc/perguntas",
      reserva_sala_status: "/marketing/reservas",
      reserva_sala_atribuicao: "/marketing/reservas",
      aprovacao_financeiro: "/financeiro",
  ...
  const handleMarkAllAsRead = async () => {
    ...
    try {
      ...
      fetchNotificacoes();
      setToastInfo({
        message: "Notificações informativas marcadas como lidas!",
        variant: "success",
      });
      setShowToast(true);
    } catch (err) {
  ...
  const getNotificationIcon = (notificacao) => {
    const iconMap = {
      ...
      status:
        notificacao.status_autorizacao === "Recusada" ? (
      ...
      dtc_entregue: <FaCheckCircle className="text-success" />,
      dtc_atribuido: <FaLightbulb className="text-warning" />,
    };
  ...
  const getNotificationText = (n) => {
    switch (n.tipo) {
      case "status":
        return (
          <>
            Sua solicitação de visita em <strong>{n.nome_cliente}</strong> foi{" "}
            <Badge
              bg={n.status_autorizacao === "Autorizada" ? "success" : "danger"}
            >
              {n.status_autorizacao}
            </Badge>
            .
          </>
        );
      case "feedback":
        return (
          <>
            Ação necessária: Registre o feedback da visita em{" "}
            <strong>{n.nome_cliente}</strong>.
          </>
        );
      case "retorno":
        return (
          <>
            Lembrete de retorno para <strong>{n.nome_cliente}</strong> agendado.
          </>
        );
      case "aprovacao_gestor":
        return (
          <>
            Aprovação de visita pendente para <strong>{n.vendedor_nome}</strong>{" "}
            no cliente <strong>{n.nome_cliente}</strong>.
          </>
        );
      case "nova_noticia":
        return (
          <>
            Nova Publicação: <strong>{n.titulo}</strong>.
          </>
        );
      case "nova_pergunta_faq":
        return (
          <>
            Nova dúvida técnica: <strong>"{n.titulo}"</strong> foi atribuída a
            você.
          </>
        );
      case "resposta_faq":
        return (
          <>
            Sua pergunta <strong>"{n.titulo}"</strong> foi respondida.
          </>
        );
      case "reserva_sala_status":
        return (
          <>
            Sua reserva <strong>"{n.titulo}"</strong> foi{" "}
            <Badge
              bg={
                n.status === "Aprovado"
                  ? "success"
      ...
      case "reserva_sala_atribuicao":
        return (
          <>
            Você foi definido como apresentador para a reserva{" "}
      ...
      case "aprovacao_financeiro":
        return (
          <>
            Aprovação financeira pendente para <strong>{n.nome_cliente}</strong>
            .
          </>
        );
      case "status_financeiro":
        return (
          <>
            Sua solicitação de <strong>{n.nome_cliente}</strong> foi atualizada.
          </>
        );
      default:
        return <span>Notificação desconhecida.</span>;
      case "dtc_entregue":
        return (
          <>
            Proposta técnica pronta: <strong>{n.titulo}</strong>.
          </>
        );
      case "dtc_atribuido":
        return (
          <>
            Novo projeto DTC: <strong>{n.titulo}</strong>.
          </>
        );
    }
  };
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Toast.Body
            className={
              toastInfo.variant === "light" || toastInfo.variant === "warning"
                ? ""
                : "text-white"
            }
          >
