...
import LancarFeriasRetroativasModal from "../components/ferias/LancarFeriasRetroativasModal";
...
const EmployeeDetailsPage = () => {
  ...
  const [showLancarRetroativoModal, setShowLancarRetroativoModal] =
    useState(false);
  ...
  const fetchEmployeeDetails = useCallback(async () => {
    ...
    setError(null);
    try {
      // 1. Busca os dados do colaborador PRIMEIRO (obrigatório para a página abrir)
      const employeeRes = await apiClient.get(
        `/api/funcionarios/${employeeId}`,
      );
      setEmployee(employeeRes.data);

      // 2. Tenta buscar as férias separadamente
      try {
        const [periodosRes, saldoRes] = await Promise.all([
          apiClient.get(
            `/api/ferias/funcionarios/${employeeId}/periodos-aquisitivos`,
          ),
        ...
        console.warn("Aviso: Dados de férias retiveram erro:", feriasErr);
        // Captura o motivo real (ex: "Sem data de admissão") e salva em um estado diferente
        // (Você precisará criar um const [feriasError, setFeriasError] = useState(null))
        // setFeriasError(feriasErr.response?.data?.error || "Erro ao carregar dados de férias.");

        setPeriodosAquisitivos([]);
        setSaldoFerias(null);
      }
    } catch (err) {
      console.error("Erro crítico ao carregar colaborador:", err);
      setError(
        "Erro ao carregar o perfil do colaborador. Verifique a conexão.",
      );
    } finally {
      setLoading(false);
    }
    ...
    const fetchAllDropdownData = async () => {
      try {
        ...
        setDropdownData({
          ...
          verticais: verticaisRes.data,
          subgrupos: subgruposRes.data,
          timesList: timesRes.data,
          beneficiosList: beneficiosRes.data,
          centrosCusto: centrosCustoRes.data,
          employees: employeesRes.data,
        });
      } catch (err) {
        console.error(
          "Erro ao carregar dados de configuração para edição:",
          err,
        );
        setError(
          "Não foi possível carregar os dados de configuração para o modal de edição.",
        );
      }
    };
    // Chama a função
  ...
  const handleRecalcularPeriodos = useCallback(async () => {
    if (!employeeId) return;
    setIsRecalculando(true);
    setError(null);
    try {
      await apiClient.post(
        `/api/ferias/funcionarios/${employeeId}/gerar-periodos`,
      );
      setSuccessMessage("Períodos aquisitivos recalculados com sucesso!");
      await fetchEmployeeDetails(); // Recarrega os dados para mostrar os novos períodos
    } catch (err) {
      console.error("Erro ao recalcular períodos:", err);
      setError(err.response?.data?.error || "Erro ao recalcular períodos.");
    } finally {
  ...
  const handleDeleteExame = async (exameId) => {
    if (
      window.confirm("Tem certeza que deseja excluir este registro de exame?")
    ) {
      try {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      await apiClient.post(
        `/api/funcionarios/${employeeId}/documentos`,
        formData,
        {
  ...
  const onboardingProgress = employee?.onboarding
    ? (employee.onboarding.etapas.filter((e) => e.status === "Concluído")
        .length /
        employee.onboarding.etapas.length) *
      100
    : 0;

  const renderDocumentsList = (docType) => {
    const docs =
      employee.documentos?.filter((d) => d.tipo_documento === docType) || [];
    if (docs.length === 0)
      return (
  ...
  const PeriodoAquisitivoCard = ({ periodo }) => {
    ...
    const statusInfo = statusMap[periodo.status] || {
      bg: "dark",
      ...
      <Card className="mb-3 shadow-sm">
        <Card.Header className="d-flex justify-content-between align-items-center">
          <span className="fw-bold">
            {periodo.numero_periodo}º Período Aquisitivo
          </span>
        ...
        <Card.Body>
          <Row>
            <Col md={6}>
              ...
              <p className="fw-semibold">
                {new Date(periodo.data_inicio_pa).toLocaleDateString("pt-BR", {
                  timeZone: "UTC",
                })}{" "}
                a{" "}
                {new Date(periodo.data_fim_pa).toLocaleDateString("pt-BR", {
                  timeZone: "UTC",
                })}
              </p>
            </Col>
            <Col md={6}>
              ...
              <p className="fw-semibold">
                {new Date(periodo.data_inicio_pc).toLocaleDateString("pt-BR", {
                  timeZone: "UTC",
                })}{" "}
                a{" "}
                {new Date(periodo.data_fim_pc).toLocaleDateString("pt-BR", {
                  timeZone: "UTC",
                })}
              </p>
            </Col>
          ...
          <div className="mt-2">
            <div className="d-flex justify-content-between small mb-1">
              <span>
                Saldo: <strong>{periodo.dias_saldo}</strong> de{" "}
        ...
        {periodo.solicitacoes.length > 0 && (
          <ListGroup variant="flush">
            <ListGroup.Item className="small fw-bold bg-light">
              Férias Agendadas/Gozadas neste Período:
            </ListGroup.Item>
            {periodo.solicitacoes.map((sol) => (
              ...
              >
                ...
                <Badge bg={STATUS_CONFIG[sol.status]?.bg || "secondary"}>
                  {STATUS_CONFIG[sol.status]?.label || sol.status}
                </Badge>
              </ListGroup.Item>
            ))}
          </ListGroup>
  ...
  return (
    <div className="container-main p-4">
      ...
      <div className="page-header-colored mb-4">
        <div className="page-header-title-wrapper">
          ...
          <p className="page-header-subtitle">
            Visualize as informações de {employee.nome_completo}.
          </p>
        </div>
        <div className="page-header-actions-wrapper">
          ...
          <Link to="/rh?tab=colaboradores" className="btn btn-light">
      ...
      <Row>
        <Col md={4} lg={3}>
          <Card className="employee-badge-card mb-4">
            <Card.Body className="text-center">
              <Image
                src={
                  employee.userpic_url ||
                  `https://ui-avatars.com/api/?name=${employee.nome_completo}&background=random`
                }
                roundedCircle
                className="mb-3"
                style={{
              ...
              <p className="text-muted">
                {employee.cargo?.nome_cargo || "Cargo não definido"}
              </p>
            </Card.Body>
            <ListGroup variant="flush">
              {renderInfoItem(
                <FaSitemap />,
                "Departamento",
                employee.setor?.nome_setor,
              )}
              {renderInfoItem(<FaUsers />, "Time", employee.time?.nome)}
              {renderInfoItem(
                <FaBuilding />,
                "Filial",
                employee.unidade?.nome_unidade,
              )}
              {renderInfoItem(
                <FaCalendarAlt />,
                "Admissão",
              ...
              {renderInfoItem(
                <FaUserTie />,
                "Gestor",
                employee.gestor?.nome_completo,
              )}
              {renderInfoItem(
                <FaDollarSign />,
                "Centro de Custo",
                employee.centro_custo?.nome,
              )}
        ...
        <Col md={8} lg={9}>
          ...
          >
              ...
              <Row>
                        ...
                        employee.formacoes.map((formacao) => (
                          <ListGroup.Item key={formacao.id}>
                            <div className="d-flex justify-content-between">
                              <span className="fw-bold">
                                {formacao.titulo_curso || "Não especificado"}
                              </span>
                              <Badge bg="secondary">
                ...
                <Col lg={4}>
                  ...
                  <Card className="mb-4">
                    ...
                    <Card.Body>
                      {employee.beneficios_atribuidos &&
                      employee.beneficios_atribuidos.length > 0 ? (
                        <div className="d-flex flex-wrap gap-2">
                          {employee.beneficios_atribuidos.map((item) => (
                            <OverlayTrigger
                              key={item.beneficio.id}
                              placement="top"
                              overlay={
                                <Tooltip>
                                  {item.beneficio.descricao || "Sem descrição"}
                                </Tooltip>
                              }
                            >
                  ...
                  <Card className="mb-4">
                    ...
                    <Card.Body>
                      <Row>
                        ...
                        {renderField(
                          "Parentesco",
                          employee.contato_emergencia_parentesco,
                        )}
                        {renderField(
                          "Telefone",
                          employee.contato_emergencia_telefone,
                        )}
                      </Row>
                    </Card.Body>
                  </Card>
                ...
                <Col lg={12}>
                  <Card className="mb-4">
                    ...
                    <Card.Body>
                      <Row>
                        ...
                        {renderField(
                          "Tipo de Admissão",
                          employee.tipo_admissao,
                        )}
                        {renderField("Matrícula", employee.matricula)}
                        {renderField(
                          "Número do Crachá",
                          employee.numero_cracha,
                        )}
                        {renderBooleanField(
                          "Primeiro Emprego",
                          employee.primeiro_emprego,
                        )}
                        {renderField(
                          "Data Exame Admissional",
                          employee.data_exame_admissional,
                          { isDate: true },
                        )}
                      </Row>
                    </Card.Body>
                  ...
                  <Card className="mb-4">
                    ...
                    <Card.Body>
                      <Row>
                        ...
                        {renderField(
                          "Categoria (eSocial)",
                          employee.categoria_trabalhador,
                        )}
                        {renderField("Salário", employee.salario, {
                          isCurrency: true,
                        })}
                        ...
                        {renderField(
                          "Salário Válido a Partir de",
                          employee.salario_valido_a_partir,
                          { isDate: true },
                        )}
                        {renderField(
                          "Motivo do Ajuste Salarial",
                          employee.motivo_ajuste_salarial,
                        )}
                        {renderField(
                          "Descrição Salarial",
                          employee.descricao_salarial,
                        )}
                      </Row>
                    </Card.Body>
                  </Card>
                  ...
                  <Card className="mb-4">
                    ...
                    <Card.Body>
                      <Row>
                        {renderBooleanField(
                          "Possui Registro de Ponto",
                          employee.possui_registro_ponto,
                        )}
                        {renderField(
                          "Hora Contratual",
                          employee.hora_contratual,
                        )}
                        {renderField(
                          "Jornada de Trabalho",
                          employee.jornada_trabalho,
                        )}
                        {renderField("Tipo de Jornada", employee.tipo_jornada)}
                        {renderField(
                          "Regime de Jornada",
                          employee.regime_jornada,
                        )}
                        {renderField("Tipo de Horário", employee.tipo_horario)}
                        {renderBooleanField(
                          "Adicional Noturno",
                          employee.hora_noturna,
                        )}
                        {renderField("Horas Mensais", employee.horas_mensais)}
                        {renderField(
                          "Descanso Semanal",
                          employee.descanso_semanal_remunerado,
                        )}
                        {renderField(
                          "Horário Detalhado",
                          employee.horario_trabalho_detalhado,
                        )}
                        {renderField(
                          "Motivo da Jornada",
                          employee.motivo_jornada,
                        )}
                        {renderField(
                          "Observação da Jornada",
                          employee.observacao_jornada,
                        )}
                      </Row>
                    </Card.Body>
                  </Card>
                  ...
                  <Card className="mb-4">
                    <Card.Header className="fw-bold">
                      Informações Legais, Sindicais e eSocial
                    </Card.Header>
                    <Card.Body>
                      <Row>
                        ...
                        {renderBooleanField(
                          ...
                          employee.contribuicao_sindical,
                        )}
                        {renderField("Estabilidade", employee.estabilidade)}
                        {renderBooleanField(
                          "Cargo de Confiança",
                          employee.cargo_confianca,
                        )}
                        {renderBooleanField(
                          "Recebeu Seguro Desemprego",
                          employee.tem_seguro_desemprego,
                        )}
                        {renderBooleanField("Aposentado", employee.aposentado)}
                        {renderField(
                          "Inscrição Órgão de Classe",
                          employee.inscricao_orgao_classe,
                        )}
                        {renderField(
                          "Conselho Profissional",
                          employee.conselho_profissional,
                        )}
                        {renderBooleanField("Membro da CIPA", employee.cipa)}
                        {renderField(
                          "Regime Previdenciário",
                          employee.tipo_regime_previdenciario,
                        )}
                        {renderField(
                          "Natureza da Atividade",
                          employee.natureza_atividade,
                        )}
                        {renderField(
                          "Indicativo de Admissão",
                          employee.indicativo_admissao,
                        )}
                        {renderBooleanField(
                          "Preenche Cota PCD",
                          employee.preenche_cota_pcd,
                        )}
                        {renderField("Agente Nocivo", employee.agente_nocivo)}
                        {renderBooleanField(
                          "Optante FGTS",
                          employee.optante_fgts,
                        )}
                        {renderBooleanField(
                          "Possui Imóvel Próprio",
                          employee.possui_imovel_proprio,
                        )}
                        {renderBooleanField(
                          "Imóvel Adquirido com FGTS",
                          employee.imovel_adquirido_fgts,
                        )}
                      </Row>
                    </Card.Body>
                  </Card>
              ...
              <Row>
                <Col lg={6} className="mb-4">
                  <Card>
                    ...
                    <ListGroup variant="flush">
                      {renderDocumentsList("CPF")}
                    </ListGroup>
                    <Card.Footer>
                      <Button
                        variant="outline-secondary"
                        size="sm"
                        onClick={() => handleUploadClick("CPF")}
                        disabled={uploading && currentDocType === "CPF"}
                      >
                        {uploading && currentDocType === "CPF" ? (
                          "Enviando..."
                        ) : (
                          <>
                  ...
                  <Card>
                    ...
                    <Card.Body>
                      <Row>
                        ...
                        {renderField("Emissão", employee.rg_data_emissao, {
                          isDate: true,
                        })}
                        {renderField(
                          "Órgão Emissor",
                          employee.rg_orgao_emissor,
                        )}
                        {renderField("UF", employee.rg_uf_emissor)}
                      </Row>
                    </Card.Body>
                    <ListGroup variant="flush">
                      {renderDocumentsList("RG")}
                    </ListGroup>
                    <Card.Footer>
                      <Button
                        variant="outline-secondary"
                        ...
                        disabled={uploading && currentDocType === "RG"}
                      >
                        {uploading && currentDocType === "RG" ? (
                          "Enviando..."
                        ) : (
                          <>
                            <FaFileUpload className="me-2" />
                  ...
                  <Card>
                    ...
                    <Card.Body>
                      <Row>
                        ...
                        {renderField("Emissão", employee.data_emissao_ctps, {
                          isDate: true,
                        })}
                    ...
                    <ListGroup variant="flush">
                      {renderDocumentsList("CTPS")}
                    </ListGroup>
                    <Card.Footer>
                      <Button
                        variant="outline-secondary"
                        size="sm"
                        onClick={() => handleUploadClick("CTPS")}
                        disabled={uploading && currentDocType === "CTPS"}
                      >
                        {uploading && currentDocType === "CTPS" ? (
                          "Enviando..."
                        ) : (
                          <>
                            <FaFileUpload className="me-2" />
                  ...
                  <Card>
                    ...
                    <Card.Body>
                      <Row>
                        ...
                        {renderField("Emissão", employee.cnh_data_emissao, {
                          isDate: true,
                        })}
                        {renderField("Validade", employee.cnh_validade, {
                          isDate: true,
                        })}
                      </Row>
                    </Card.Body>
                    <ListGroup variant="flush">
                      {renderDocumentsList("CNH")}
                    </ListGroup>
                    <Card.Footer>
                      <Button
                        variant="outline-secondary"
                        ...
                        disabled={uploading && currentDocType === "CNH"}
                      >
                        {uploading && currentDocType === "CNH" ? (
                          "Enviando..."
                        ) : (
                          <>
                            <FaFileUpload className="me-2" />
                ...
                <Col lg={6} className="mb-4">
                  <Card>
                    ...
                    <Card.Body>
                      <Row>
                        {renderField("Número", employee.titulo_eleitor_numero)}
                        {renderField("Zona", employee.titulo_eleitor_zona)}
                        {renderField("Seção", employee.titulo_eleitor_secao)}
                      </Row>
                    </Card.Body>
                    <ListGroup variant="flush">
                      {renderDocumentsList("TITULO_ELEITOR")}
                    </ListGroup>
                    <Card.Footer>
                      <Button
                        variant="outline-secondary"
                        size="sm"
                        onClick={() => handleUploadClick("TITULO_ELEITOR")}
                        disabled={
                          uploading && currentDocType === "TITULO_ELEITOR"
                        }
                      >
                        {uploading && currentDocType === "TITULO_ELEITOR" ? (
                          "Enviando..."
                ...
                <Col lg={6} className="mb-4">
                  <Card>
                    ...
                    <Card.Body>
                      <Row>
                        {renderField("Número", employee.reservista_numero)}
                        {renderField("RA", employee.reservista_ra)}
                        {renderField(
                          "Categoria",
                          employee.reservista_categoria,
                        )}
                      </Row>
                    </Card.Body>
                    <ListGroup variant="flush">
                      {renderDocumentsList("RESERVISTA")}
                    </ListGroup>
                    <Card.Footer>
                      <Button
                        variant="outline-secondary"
                        size="sm"
                        onClick={() => handleUploadClick("RESERVISTA")}
                        disabled={uploading && currentDocType === "RESERVISTA"}
                      >
                        {uploading && currentDocType === "RESERVISTA" ? (
                          "Enviando..."
                        ) : (
                          <>
                            <FaFileUpload className="me-2" />
                ...
                <Col lg={12} className="mb-4">
                  <Card>
                    <Card.Header className="d-flex justify-content-between align-items-center fw-bold">
                      ...
                      >
                        <FaPlus className="me-1" /> Adicionar Exame
                      </Button>
                    </Card.Header>
                    <ListGroup variant="flush">
                      {employee.exames_periodicos &&
                      employee.exames_periodicos.length > 0 ? (
                        ...
                        <ListGroup.Item className="text-muted">
                          Nenhum exame periódico registrado.
                        </ListGroup.Item>
                      )}
                    </ListGroup>
                  </Card>
                ...
                <Col lg={6} className="mb-4">
                  <Card>
                    <Card.Header className="fw-bold">Recibos</Card.Header>
                    <ListGroup variant="flush">
                      {renderDocumentsList("RECIBOS")}
                    </ListGroup>
                    <Card.Footer>
                      <Button
                        variant="outline-secondary"
                        size="sm"
                        onClick={() => handleUploadClick("RECIBOS")}
                        disabled={uploading && currentDocType === "RECIBOS"}
                      >
                        {uploading && currentDocType === "RECIBOS" ? (
                          "Enviando..."
                        ) : (
                          <>
                            <FaFileUpload className="me-2" />
                ...
                <Col lg={6} className="mb-4">
                  <Card>
                    ...
                    <ListGroup variant="flush">
                      {renderDocumentsList("TERMOS_CONTRATOS")}
                    </ListGroup>
                    <Card.Footer>
                      <Button
                        variant="outline-secondary"
                        size="sm"
                        onClick={() => handleUploadClick("TERMOS_CONTRATOS")}
                        disabled={
                          uploading && currentDocType === "TERMOS_CONTRATOS"
                        }
                      >
                        {uploading && currentDocType === "TERMOS_CONTRATOS" ? (
                          "Enviando..."
                ...
                <Col lg={6} className="mb-4">
                  <Card>
                    <Card.Header className="fw-bold">Recessos</Card.Header>
                    <ListGroup variant="flush">
                      {renderDocumentsList("RECESSOS")}
                    </ListGroup>
                    <Card.Footer>
                      <Button
                        variant="outline-secondary"
                        size="sm"
                        onClick={() => handleUploadClick("RECESSOS")}
                        disabled={uploading && currentDocType === "RECESSOS"}
                      >
                        {uploading && currentDocType === "RECESSOS" ? (
                          "Enviando..."
                        ) : (
                          <>
                            <FaFileUpload className="me-2" />
                ...
                <Col lg={6} className="mb-4">
                  <Card>
                    ...
                    <ListGroup variant="flush">
                      {renderDocumentsList("DOCUMENTOS_ADICIONAIS")}
                    </ListGroup>
                    <Card.Footer>
                      <Button
                        variant="outline-secondary"
                        size="sm"
                        onClick={() =>
                          handleUploadClick("DOCUMENTOS_ADICIONAIS")
                        }
                        disabled={
                          uploading &&
                          currentDocType === "DOCUMENTOS_ADICIONAIS"
                        }
                      >
                        {uploading &&
                        currentDocType === "DOCUMENTOS_ADICIONAIS" ? (
            ...
            <Tab
              eventKey="historico-profissional"
                    ...
                    {employee.historico_profissional?.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan="7" className="text-center text-muted">
                          Nenhum histórico profissional encontrado.
                        </td>
                      </tr>
                    )}
                  </tbody>
                    ...
                    {employee.historico_alteracoes?.length > 0 ? (
                      employee.historico_alteracoes.map((log) => (
                        <tr key={log.id}>
                          ...
                          <td>
                            {log.alterado_por?.nome_completo || "Sistema"}
                          </td>
                        </tr>
                      ))
                    ) : (
            ...
            <Tab eventKey="onboarding" title="Onboarding">
              {employee.onboarding ? (
                <Card>
                  <Card.Header>
                    <div className="d-flex justify-content-between align-items-center">
                      ...
                      <span className="fw-bold">
                        {onboardingProgress.toFixed(0)}%
                      </span>
                    </div>
                  </Card.Header>
                  <Card.Body>
                    <ProgressBar
                      now={onboardingProgress}
                      label={`${onboardingProgress.toFixed(0)}%`}
                      className="mb-4"
                    />
                    <ListGroup>
                      {employee.onboarding.etapas.map((etapa) => (
                        <ListGroup.Item key={etapa.id}>
                          <Form.Check
                            ...
                            onChange={() =>
                              handleToggleOnboardingStep(etapa.id)
                            }
                          />
                          {etapa.status === "Concluído" && (
                            <div className="text-muted small ps-4">
                              Concluído por{" "}
                              {etapa.concluido_por?.nome_completo || "N/A"} em{" "}
                              {new Date(
                ...
                <Alert variant="info">
                  Nenhum processo de onboarding iniciado para este colaborador.{" "}
                  <Button
                    variant="link"
                ...
                <>
                        ...
                        <Col md={8}>
                          <p className="mb-1">
                            ...
                            <strong className="fs-5">
                              {saldoFerias.total_dias_saldo} dias
                            </strong>
                          </p>
                          {saldoFerias.proximo_vencimento ? (
                            <p className="mb-1 text-muted small">
                              ...
                              <strong>
                                {new Date(
                                  saldoFerias.proximo_vencimento.data,
                                ).toLocaleDateString("pt-BR", {
                                  timeZone: "UTC",
                                })}
                              </strong>{" "}
                              ({saldoFerias.proximo_vencimento.dias_restantes}{" "}
                              dias restantes)
                            </p>
                          ) : (
                            <p className="text-muted small">
                              Nenhum período com vencimento próximo.
                            </p>
                          )}
                          {saldoFerias.tem_ferias_vencidas && (
                            <Alert variant="danger" className="mt-2 py-2 small">
                              Atenção! Você possui períodos de férias vencidos.
                            </Alert>
                          )}
                        </Col>
                  ...
                  <div className="d-flex gap-2 mb-3">
                    <Button
                      ...
                      size="sm"
                      onClick={() => setShowLancarRetroativoModal(true)}
                    >
                      📋 Lançar Férias Retroativas (RH)
                    </Button>
                    {/* PASSO 2: Adicionar o Botão de Recálculo */}
                    <Button
                      variant="outline-warning"
                      size="sm"
                  ...
                  <Table striped bordered hover responsive size="sm">
                    ...
                    <tbody>
                      {periodosAquisitivos.flatMap((pa) => pa.solicitacoes)
                        .length > 0 ? (
                        periodosAquisitivos
                          .flatMap((pa) => pa.solicitacoes)
                          .sort(
                            (a, b) =>
                          ...
                          .map((sol) => (
                            <tr key={sol.id}>
                              <td>
                                {new Date(sol.data_inicio).toLocaleDateString(
                                  "pt-BR",
                                  { timeZone: "UTC" },
                                )}{" "}
                                a{" "}
                              ...
                              <td>
                                <Badge
                                  bg={
                                    STATUS_CONFIG[sol.status]?.bg || "secondary"
                                  }
                                >
      ...
      />
      <LancarFeriasRetroativasModal
        show={showLancarRetroativoModal}
        onHide={() => setShowLancarRetroativoModal(false)}
        funcionarioId={employeeId}
        onSuccess={() => {
          setShowLancarRetroativoModal(false);
          // Recarrega os dados de férias para refletir o novo saldo
          fetchFeriasData(); // ajuste para o nome da sua função de reload
        }}
      />
      <DependentsModal
