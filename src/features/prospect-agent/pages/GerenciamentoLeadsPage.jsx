...
const getStatusVariant = (status) => {
  return (
    statusOptions.find((opt) => opt.value === status)?.variant || "secondary"
  );
};

const GerenciamentoLeadsPage = () => {
  // Renomeado
  ...
  const handleRunAgent = async () => {
    ...
    setError("");
    try {
      const response = await apiClient.post("/api/agente/executar-leads", {
  ...
  const handleStatusChange = async (leadId, newStatus) => {
    try {
      const response = await apiClient.put(`/api/leads/${leadId}/status`, {
        status: newStatus,
      });
      if (!response.ok) {
      ...
      <Row className="justify-content-center">
        <Col md={12}>
          ...
          <Card className="shadow-sm border-0 mb-4">
            <Card.Header as="h5">
              Prospecção de Leads (Pesquisa Rápida)
            </Card.Header>
            <Card.Body>
              {/* ================================================================= */}
                ...
                <tbody>
                  ...
                  {leads.map((lead) => (
                    <tr key={lead.id}>
                      ...
                      <td>{lead.regiao}</td>
                      <td>
                        <Badge bg="secondary">{lead.termo_pesquisa}</Badge>
                      </td>
                      <td>
                        <Form.Select
                          size="sm"
...
export default GerenciamentoLeadsPage;
