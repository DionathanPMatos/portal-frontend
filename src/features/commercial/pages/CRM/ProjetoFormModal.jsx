...
const ProjetoFormModal = ({ show, onHide, onSuccess, projetoParaEditar, defaultStage }) => {
    ...
    useEffect(() => {
        if (show) {
            ...
            const endpoints = [apiClient.get('/api/vendedores'), apiClient.get('/api/funcionarios'), apiClient.get('/api/segmentacoes'), apiClient.get('/api/verticais'), apiClient.get('/api/fabricantes'), apiClient.get('/api/integradores')];
    ...
    useEffect(() => {
        if (clienteSearch.length >= 2) {
            const fetchClientes = async () => {
                try {
                    const res = await apiClient.get(`/api/clientes?search=${clienteSearch}&limit=10`);
    ...
    const handleAddNewSegmentacao = async () => {
        ...
        try {
            const res = await apiClient.post('/api/segmentacoes', { nome: newSegText.trim() });
    ...
    const handleAddNewVertical = async () => {
        ...
        try {
            const res = await apiClient.post('/api/verticais', { nome: newVertText.trim() });
