...
export default function ModalImportarPrestador({ show, onHide, onSave }) {
  ...
  const handleDownloadTemplate = () => {
    window.open("/api/facilities/prestadores/template", "_blank");
  };
  ...
  return (
    <Modal show={show} onHide={onHide} centered>
      <Modal.Header closeButton>
        <Modal.Title>Importar Prestadores de Serviços</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {erro && <Alert variant="danger">{erro}</Alert>}
        {sucesso && <Alert variant="success">{sucesso}</Alert>}

        <p>Para importar vários prestadores de uma só vez, baixe a planilha modelo, preencha as colunas e faça o upload do arquivo salvo.</p>
