...
function ClienteEditModal({ show, onHide, value, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.put(`/api/clientes/${value.id}`, payload);
      onSaved?.();
      onHide?.();
    } catch (e) {
      console.error(e);
...
function ContatoModal({ show, onHide, value, clienteId, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await axios.put(`/api/contatos/${value.id}`, payload);
      } else {
        await axios.post(`/api/contatos`, payload);
      }
...
function FilialModal({ show, onHide, value, clienteId, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.post(`/api/clientes/${clienteId}/filiais`, payload);
...
export default function ClienteDetalhe() {
  ...
  const fetchFiliais = async () => {
    try {
      const r = await axios.get(`/api/clientes/${id}/filiais`);
      setFiliais(r.data || []);
    } catch (e) {
  ...
  const handleDeleteContato = async (contatoId) => {
    ...
    setSuccess("");
    try {
      await axios.delete(`/api/contatos/${contatoId}`);
  ...
  const handleDeleteFilial = async (filialId) => {
    ...
    setSuccess("");
    try {
      await axios.delete(`/api/clientes/filiais/${filialId}`);
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        await axios.put(`/api/clientes/${id}`, updated);
