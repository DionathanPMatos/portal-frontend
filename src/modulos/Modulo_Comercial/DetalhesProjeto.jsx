...
import PedidoErpModal from './modulo_crm/PedidoErpModal';
import '../../App.css'
import './crm.css';
...
const carregarDetalhesPedido = async () => {
        ...
        setError('');
        try {
            const resPedido = await axios.get(`/api/erp/pedido/${pedidoVinculado.numero_pedido}`);
            const resEstoque = resPedido.data.itens.length > 0
                ? await axios.post(`/api/erp/estoque`, { codigos: resPedido.data.itens.map(i => i.codigo) })
                : { data: [] };
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            const res = await axios.get(`/api/compras/solicitacao/pedido/${pedidoVinculado.id}`).catch(() => null);
    ...
    const handleSolicitarCompra = async () => {
        ...
        if (window.confirm('Confirmar o envio da solicitação de compra?')) {
            try {
                const res = await axios.post('/api/compras/solicitacoes', {
...
const DetalhesProjeto = ({ user }) => {    const { id } = useParams();
    ...
    const fetchData = async () => {
        try {
            ...
            const [projetoRes, documentosRes, atividadesRes] = await Promise.all([
                ...
                axios.get(`/api/projetos/${id}/documentos`),
    ...
    const handleAddAtividade = async (tipo, dados) => {
        ...
        try {
            ...
            const response = await axios.post(`/api/projetos/${id}/atividades`, payload);
    ...
    const handleToggleTask = async (taskId, statusAtual) => {
        ...
        try {
            await axios.patch(`/api/atividades/${taskId}/toggle`);
        } catch (err) {
...
const handleMarkAsLost = async () => {
    ...
    try {
        await axios.patch(`/api/projetos/${id}/status`, {
    ...
    const handleSolicitarProposta = async () => {
        if (window.confirm('Tem certeza?')) {
            try {
                const response = await axios.patch(`/api/projetos/${id}/solicitar-proposta`);
    ...
    const handleEnviarParaRevisao = async () => {
        if (window.confirm('Tem certeza?')) {
            try {
                const response = await axios.patch(`/api/projetos/${id}/revisar`);
                setSuccess(response.data.message);
                fetchData();
                setTimeout(() => setSuccess(''), 4000);
            } catch (err) {
    ...
    const handleFileUpload = async (event) => {
        ...
        try {
            const response = await axios.post(`/api/projetos/${id}/upload`, formData);
            setSuccess(response.data.message);
            event.target.reset();
    ...
    const handleDeleteDocument = async (docId) => {
        if (window.confirm('Tem certeza?')) {
            try {
                await axios.delete(`/api/projetos/documentos/${docId}`);
    ...
    const handleVerificarPedido = async () => {
        ...
        setImportError('');
        try {
            const res = await axios.get(`/api/erp/pedido/${numeroPedidoErp}`);
    ...
    const handleAtrelarPedido = async () => {
        ...
        setIsLinking(true);
        try {
            await axios.patch(`/api/projetos/${id}/atrelar-pedido`, { numero_pedido: pedidoImportado.numero_pedido });
    ...
    const handleDesvincularPedido = async (pedidoVinculadoId) => {
        if (window.confirm('Tem certeza que deseja desvincular este pedido do projeto?')) {
            try {
                await axios.delete(`/api/projeto-pedidos/${pedidoVinculadoId}`);
