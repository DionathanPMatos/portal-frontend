...
const RepositorioTecnico = () => {
    ...
    const fetchDocumentos = async () => {
        setLoading(true);
        try {
            const response = await axios.get('/api/repositorio');
    ...
    const handleSaveDocument = async (e) => {
        ...
        try {
            await axios.post('/api/repositorio', uploadData, {
    ...
    const handleDeleteDocument = async (id) => {
        if (window.confirm("Deseja realmente excluir este documento?")) {
            try {
                await axios.delete(`/api/repositorio/${id}`);
                                            ...
                                            filteredDocs.map((doc) => (
                                                <tr key={doc.id}>
                                                    ...
                                                    <td className="py-3 text-end px-4">
                                                        <a href={`${API_URL}/${doc.caminho_arquivo}`} target="_blank" rel="noopener noreferrer" className="btn btn-light btn-sm me-2 shadow-sm text-primary" title="Abrir Arquivo">
                                                            <FaExternalLinkAlt />
                                                        </a>
