...
import NewsWidget from "../features/news/pages/NewsWidget"; // Caminho ajustado para a feature news
import apiClient from "../services/api"; // Usa a instância configurada do Axios

function DashboardPage({ isLoggedIn }) { // Renomeado para seguir convenção de página
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      await apiClient.post('/api/upload-csv', formData, { // Usa apiClient
  ...
  const handleClearData = async () => {
    if (window.confirm('Tem certeza que deseja APAGAR TODOS os dados de vendas importados? Essa ação não pode ser desfeita.')) {
      try {
        setLoading(true);
        await apiClient.delete('/api/dashboard-metrics/clear'); // Usa apiClient
...
export default DashboardPage; // Exporta o nome atualizado